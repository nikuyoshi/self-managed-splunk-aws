...
export class ESDownloadHelper {
  ...
  static generateInstallScript(config: SplunkConfig, uploadedPath: string): string[] {
    return [
      ...
      '  ',
      '  # Stop Splunk before installing ES to avoid conflicts',
      '  echo "Stopping Splunk for ES installation..."',
      ...
      '    ',
      '    # Start Splunk after ES installation',
      '    echo "Starting Splunk with ES..."',
      '    sudo -u splunk /opt/splunk/bin/splunk start',
      '  else',
      '    echo "❌ ERROR: Failed to install Enterprise Security (exit code: $ES_INSTALL_RESULT)"',
      '    echo "Attempting to start Splunk without ES..."',
      '    sudo -u splunk /opt/splunk/bin/splunk start',
