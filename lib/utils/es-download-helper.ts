...
export class ESDownloadHelper {
  ...
  static generateInstallScript(config: SplunkConfig, uploadedPath: string): string[] {
    return [
      ...
      `  /opt/splunk/bin/splunk install app ${uploadedPath} -auth admin:$ADMIN_PASSWORD`,
      '  if [ $? -eq 0 ]; then',
      '    echo "Enterprise Security installed successfully"',
      '    # Configure ES-specific settings',
      '    /opt/splunk/bin/splunk set servername es-search-head -auth admin:$ADMIN_PASSWORD',
  ...
  static generateHealthCheckScript(): string[] {
    return [
      ...
      'if /opt/splunk/bin/splunk list app -auth admin:$ADMIN_PASSWORD | grep -q "SplunkEnterpriseSecuritySuite"; then',
