...
export class SplunkClusterStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SplunkClusterStackProps) {
    ...
    this.clusterManager.userData.addCommands(
      ...
      '/opt/splunk/bin/splunk edit cluster-config -mode manager -replication_factor 3 -search_factor 2 -secret clustersecret -auth admin:$ADMIN_PASSWORD || echo "Failed to configure cluster manager"',
    ...
    this.indexerAsg.addUserData(
      ...
      '  set +e',
      '  OUTPUT=$(/opt/splunk/bin/splunk edit cluster-config -mode peer -manager_uri https://$CLUSTER_MANAGER_IP:8089 -replication_port 9100 -secret clustersecret -auth admin:$ADMIN_PASSWORD 2>&1)',
      '  EXITCODE=$?',
      '  set -e',
      '  echo "$OUTPUT"',
      '  ',
