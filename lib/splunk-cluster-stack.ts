...
export class SplunkClusterStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SplunkClusterStackProps) {
    ...
    this.clusterManager.userData.addCommands(
      ...
      'cat << EOF | sudo -u splunk tee /opt/splunk/etc/system/local/user-seed.conf > /dev/null',
      '[user_info]',
      'USERNAME = admin',
      ...
      `echo "Replication Factor: ${config.replicationFactor}, Search Factor: ${config.searchFactor}"`,
      `sudo -u splunk /opt/splunk/bin/splunk edit cluster-config -mode manager -replication_factor ${config.replicationFactor} -search_factor ${config.searchFactor} -secret clustersecret -auth admin:$ADMIN_PASSWORD`,
      'if [ $? -eq 0 ]; then',
