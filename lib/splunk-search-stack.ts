...
export class SplunkSearchStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SplunkSearchStackProps) {
    ...
    this.searchHead.userData.addCommands(
      ...
      '  exit 1',
      'fi',
      'echo "Admin password retrieved successfully (length: ${#ADMIN_PASSWORD})"',
      '',
      ...
      '# Use echo with proper variable expansion instead of heredoc to ensure password is set correctly',
      'echo "[user_info]" | sudo -u splunk tee /opt/splunk/etc/system/local/user-seed.conf > /dev/null',
      'echo "USERNAME = admin" | sudo -u splunk tee -a /opt/splunk/etc/system/local/user-seed.conf > /dev/null',
      'echo "PASSWORD = ${ADMIN_PASSWORD}" | sudo -u splunk tee -a /opt/splunk/etc/system/local/user-seed.conf > /dev/null',
      '',
      '# Verify the user-seed.conf was created with password',
      'echo "Verifying user-seed.conf creation..."',
      'if ! sudo -u splunk grep -q "PASSWORD = ." /opt/splunk/etc/system/local/user-seed.conf; then',
      '  echo "ERROR: user-seed.conf does not contain a password"',
      '  echo "Content of user-seed.conf:"',
